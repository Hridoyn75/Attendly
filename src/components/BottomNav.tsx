...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  return (
    <div className="fixed bottom-3.5 left-0 right-0 z-40 px-4 pointer-events-none">
      <nav className="max-w-xs mx-auto glass-panel border border-zinc-200/80 p-1.5 rounded-full shadow-xl shadow-indigo-950/5 pointer-events-auto flex items-center justify-between font-sans bg-white/90 backdrop-blur-md transform-gpu">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex-1 flex items-center justify-center space-x-1.5 py-2 px-3 rounded-full transition-all duration-150 ease-out select-none cursor-pointer outline-none transform-gpu ${
                isActive
                  ? 'bg-indigo-600 text-white font-black shadow-md shadow-indigo-600/25 scale-[1.02]'
